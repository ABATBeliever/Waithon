...
tok_get(struct tok_state *tok, const char **p_start, const char **p_end)
{
 ...
 again:
    ...
    /* Waithon */
    /* if (memcmp(tok->start, "hogehoge", cnt) == 0) cntにはhogehogeのバイト数を入力 python -c "print(len('hogehoge'.encode()))" */
    /*     return WAITHON_HOGE; WAITHON_NAMEは命令の名前。 */
        if (memcmp(tok->start, "もしも", 9) == 0)
            return WAITHON_MOSHIMO;
        if (memcmp(tok->start, "のとき", 9) == 0)
            return WAITHON_NOTOKI;
        if (memcmp(tok->start, "ではなく", 12) == 0)
            return WAITHON_DEWANAKU;
        if (memcmp(tok->start, "ではないとき", 18) == 0)
            return WAITHON_DEWANAITOKI;
        return NAME;
    }
